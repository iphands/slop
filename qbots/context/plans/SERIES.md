...
| **17** | BSP/collision hardening & STEP fix | 05 | pending | Fresh vendor re-audit (2026-06-16) confirmed the shipped model-margin fix is correct and found one new bug: nav graph `STEP=24` vs real `STEPSIZE=18` (`pmove.c:32`) → bots can't actually climb height diffs the graph calls walkable. Also: entity-tokenizer `//` comment parity, vendor-constant pin tests, backfill `pitfalls.md`. |
| **18** | Ahead-of-time map cache (`generate-map-cache`) | 17, 09 | pending | Move BSP/collision/nav-graph build out of every bot's connect path into a fingerprinted on-disk cache (`./data/mapcache/<map>.qnav`), generated by a new CLI command supporting single map or `q2dm*` wildcard, parallelized (rayon within a map, N-maps-at-once across maps). |
