...
| **41** | `spawn-to-item` + item/weapon target resolution | 10, 26 | pending | New `spawn-to-item <item>` movement scenario + friendly item aliases (`quaddamage`→`item_quad`) + `--instance N` so multi-instance targets are selectable (q2dm3 has **two** `weapon_railgun`). Pure CLI/goal-resolution; unblocks q2dm3 validation for 42/43. |
| **42** | Moving-platform (`func_train`) + lift nav integration (q2dm3) | 17, 18, 39, 34, 35 | pending | `EdgeKind::Ride` train edges (board→dismount) + `func_train`/`path_corner` parse + lift top-landing anchoring so q2dm3 quad `(192,320,216)` + loop-train railgun `(768,816,208)` join the reachable graph → `generate-map-cache q2dm3`. Coordinates with Plan 35 (broad `walkable_stair` regression) for full 7/7. Cache VERSION bump. |
| **43** | Moving-platform & lift **ride behavior** + q2dm3 reach proof / navmode ranking | 42, 41, 40, 26 | pending | Brain executes `Ride` edges: approach→wait→board→ride→dismount (live platform origin from PVS), recovery suspended (recorder `P`), railgun `func_plat` ride (single-bot; multi-bot de-conflict stays Plan 31). Live proof of `spawn-to-item quaddamage` + `spawn-to-weapon railgun --instance 1` reaching, ranked across 6 navmodes in `mode_perf.md`. |
