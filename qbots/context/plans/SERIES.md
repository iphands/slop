...
| **35** | q2dm nav connectivity (fragmentation) | 34 | in-progress | **Root cause was missing connector mechanisms, not a `walkable_stair` regression** (the prune is innocent — fragmentation persists with `QBOTS_NO_PRUNE=1`). Fixed via new nav edges: **ladder climbs** (`CONTENTS_LADDER`, the key for q2dm3's upper level), **func_train/lift ride edges** (Plan 42), and **vertical jump-down floor bridges**. **q2dm3 now 7/7** (was 3/7); q2dm1/2/4/5/8 full; q2dm7 3/6→4/6; q2dm6 7/8. **Remaining:** ladder-climb-finish tuning (bot stalls ~40u below ladder top), q2dm6/7 residual, and the q2dm3 quad's specific platform-jump entry. |
...
| **44** | **3ZB2-style brain** (MVP) | 43, 06 | pending | Port 3ZB2's route-following + shortcut optimization + state machine for elevators/doors/trains. **Deliverable:** Pluggable `Brain` implementation with battle-tested navigation. Competition vs `q3` brain (Plan 37). |
| **45** | `main` brain competitiveness vs `q3` | 24, 37 | stopped (partial) | Improve `main` to beat `q3`-major (hybrid-fallback) via strategy only, `q3` untouched. Shipped (2 commits): fire-cadence fix (0.9→0.2s switch lockout, reaction base halved; `combat.rs`, main+sentry only), weapon-rush (Blaster→evade+grab a gun; hitscan→stand&delete), loadout/health-weighted item picker (`items::best_item_goal_weighted`, main-only), fast combat strafe juke (`STRAFE_PERIOD_SECS` 3.0→0.6s, main-only). **Result: main kd 0.47→0.68, deaths 38→25** — not a win over q3 (~1.3). Key finding: main already has perfect hitscan aim + faster reaction than q3, so the residual gap is per-engagement combat quality, not tactics; positioning/flee tuning plateaus ~0.5–0.68. **Stopped by user decision** at the ~45% gain (closing the rest needs a combat-strength change, out of scope). Reverted-worse experiments logged in the tracker. |
