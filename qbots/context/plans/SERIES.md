...
| **39** | Water nav graph (swim nodes + swim edges) | 05, 17, 18 | pending | Teach the A* `NavGraph` to traverse water: `EdgeKind::Swim`, submerged/surface node sampling, swim↔swim + dry↔water entry/exit edges, cache fingerprint bump, `navinspect contents/watermap`. **Root cause confirmed 2026-06-19**: q2dm1 railgun `(240,-384,464)` is reachable only by swimming, but all water positions are discarded in nav gen (`navgraph.rs:1200`, `heightfield.rs:258`) → railgun room is an isolated component (`navinspect navpath` = NO PATH from every spawn). Offline-verifiable (path Some). Navmesh water deferred. |
| **40** | Swim movement, water-exit & navmode ranking | 39, 10, 12, 13, 26 | pending | Make the brain execute swim edges: water-state detection (computed from our own `cm`+origin), `intent.up`/pitch vertical thrust, Q2 water-jump climb-out onto the railgun ledge, water-aware recovery, recorder `S` flag. **No brain sets `intent.up` today**; recovery actively avoids water. Proof = `spawn-to-weapon railgun --count 1 --max-secs 300 --navmode <m>` across all six navmodes + a reached/elapsed ranking table. Folds toward Plan 32 (breath). |
...
> **Brain-notes discipline (Plans 23–33, 36–38, 40):** every brain plan appends a dated section to
