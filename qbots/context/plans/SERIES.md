...
| **27** | Persona parameters (behavior) | 25 | pending | **Plan file authored 2026-07-09.** `Persona` traits ([0,1], q3char-style) + 4 named presets (`rusher`/`sniper`/`scavenger`/`guard`), `--persona`/`--personas`/fleet-config plumbing (mirror Plan 38's `--q3char`); `main`'s tactical consts (`FLEE_HEALTH`, `KITE_HEALTH`, strafe period, dwell) become persona lookups; default persona byte-preserves current behavior (unit-tested). |
| **28** | Tactical weapon-matchup reads (behavior) | 27 | pending | **Plan file authored 2026-07-09.** Distance *switching* already exists (`weapons.rs:206`, `use` stringcmds) — the gaps: infer enemy weapon from VWep `modelindex2`→CS_MODELS (verify VWep live first), per-weapon ideal-**range positioning** replacing fixed `IDEAL_DIST`/`BACKUP_DIST`, `matchup_score`-gated engage bias. The other half of "switch weapons for close/far combat". |
| **29** | Engagement: chase / disengage / third-party (behavior) | 28, 27 | pending | **Plan file authored 2026-07-09.** "Chase for the kill": pursuit-capable `Hunt` (velocity-extrapolated, A*-pathed, persona/matchup-budgeted), `EngageRead` winning/losing estimator (enemy health NOT on wire — pressure + own-health trend), **third-party 1v1 break**. q3 untouched (control brain). |
| **30** | Resource decisions: health & ammo (behavior) | 24, 41 | pending | **Plan file authored 2026-07-09.** Item seeking is PVS-blind today (`items.rs`) — add `BrainMap.items` (static BSP item table), per-bot taken/respawn memory (PVS-honest), **hurt → nearest *reachable* (A*-distance) health** (Flee seeks health, the literal "collect health when hurt"), ammo-aware weapon scoring (dry weapon scores 0) + re-arm goals. |
...
| **35** | q2dm nav connectivity: hull-valid routes + residual gaps | 34, 42, 43 | in-progress | **Revised 2026-07-09** (the 06-19 far-spawn deferral is superseded by the user's human-like-navigation directive). Root cause resolved (missing connectors — ladders/rides/jump-bridges shipped; ladder ascent top-out fixed `31c3ee57a`; q2dm3 7/7; q2dm1/2/4/5/8 full). **Remaining scope:** (T1) hull-validate + split bridge/seed edges (the 354u hull-blocked "walk" bridges that break far-spawn→quad routes), (T2) resample the fragmented q2dm3 upper level → `spawn-to-item quaddamage --count 4` ≥ 3/4 from mixed spawns, (T3) q2dm6 7/8→8/8 + q2dm7 4/6→≥5/6, (T4) regen + VERSION bump. |
...
| **42** | Moving-platform (`func_train`) + lift nav integration (q2dm3) | 17, 18, 39, 34 | in-progress (~95%) | `EdgeKind::Ride`+`RideInfo`, `func_train` ride edges (ground-anchored boards), lifts→**vertical ride edges**, `bridge_components_via_jump`, cache **v18** (two-height ride search `13b08c4ae` + `stand_offset` `4cfdd2cb8`). **Railgun AND quad both A\*-reachable from all 7 q2dm3 spawns.** **Remaining: T6 only** — gated offline reachability test (path exists + contains a `Ride` edge, mirror `water_q2dm1.rs`) → then move to `completed/`. |
| **43** | Moving-platform & lift **ride behavior** + q2dm3 reach proof / navmode ranking | 42, 41, 40, 26 | in-progress (~90%) | Ride behavior **SOLVED**: approach/wait/board/carry/dismount with JUMP on/off, wire-origin train detection, live top-center tracking, `active_ride` edge lock, zero-input carry. **q2dm3 railgun REACHED (astar 3/4, hybrid-race 3/4); QUAD REACHED from spawn3** (`ff155abfd`, rides `*10` over the lava). Far-spawn quad routes = Plan 35 (revised). **Remaining: T4 recorder `P` flag + T6 complete the 6-navmode ranking rows** → then move to `completed/`. |
...
| **46** | **Shared traversal executor** (ladder/swim/ride parity for ALL brains) | 40, 43, 24/25 | pending | **New 2026-07-09 — the keystone for human-like map navigation in live matches.** Today: `runtester` has the full set (incl. the only ladder machine), `main` has swim + generic ride but **no ladders**, **`q3` has nothing** (can't swim/ride/climb in matches). Extract one `brain::traverse::TraversalExecutor` (movement-only override, aim/fire stays live, recovery suspension, recorder `S`/`P`/`L`); all brains adopt; duplicates deleted. Behavior-preserving extraction (Plan 22 discipline) with a live regression matrix. |
