...
| **10** | Movement test harness | 09 | pending | BSP **entity-lump** parse (spawn points + `weapon_*` origins), `spawn-to-spawn` / `spawn-to-weapon` CLI, per-frame `MovementRecorder` → structured `./logs/<scenario>/<ts>.<bot>.log` → **measure pathing accuracy & elapsed time** (the lens for 11-14) |
| **11** | Honest LOS perception | 10 | pending | BSP-trace line-of-sight gate on enemy selection (`combat.rs`), FSM transition (`fsm.rs`), fire/chase, and nav-to-enemy → **bots stop targeting/shooting/walking through walls** |
| **12** | Steering controller | 11 | pending | turn-rate limiting, look-ahead/"pursue" target + arrive, anti-orbit Z-aware node-advance, facing-validated forward, aim/move-yaw separation (circle-strafe) → **bots move like humans, not orbit/spin/wedge** |
