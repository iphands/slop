...
| **22** | Brain seam extraction | 06, 07, 12, 13 | in-progress | Extract the dissolved decision/steering body of `bot_task` (`main.rs:818–1111`) into a single `brain::Brain` (owns combat/fsm/danger/steering/recovery/skill/roam; `Navigator` injected per tick). `bot_task` becomes thin orchestration. **Zero behavior change** — Plan 10 SUMMARY-line parity. Establishes the seam for behavior/persona work without touching nav or the driver. |
| **23** | Behavior/persona + nav-leak pull-up | 22 | pending | Phase 2 on the Brain seam: per-bot personas (aggression/weapon-pref/follow/reaction), real elevator behavior (decide+wait+ride; **remove `ELEVATOR_PENALTY`**, nav exposes plat facts only), heatmap *preference* moved up to Brain (nav exposes per-node danger), and explicit persona-tuned tactics (e.g. back-up vs the SSG instead of fixed `BACKUP_DIST`). |
