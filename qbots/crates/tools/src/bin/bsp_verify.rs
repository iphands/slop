...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, sp) in spawn_points.iter().enumerate() {
        println!(
            "    spawn[{}]: ({:.0}, {:.0}, {:.0})",
            i, sp.origin[0], sp.origin[1], sp.origin[2]
        );
    }
    ...
    for (name, point) in &test_points {
        ...
        println!(
            "  {} at ({:.0}, {:.0}, {:.0}): contents={:#x} (solid={}, water={})",
    ...
    println!(
        "  Trace from ({:.0}, {:.0}, {:.0}) to ({:.0}, {:.0}, {:.0}):",
        top[0], top[1], top[2], bot[0], bot[1], bot[2]
    );
    println!(
        "    fraction={:.3}, startsolid={}, endpos=({:.0}, {:.0}, {:.0})",
    ...
    println!(
        "    fraction={:.3}, startsolid={}",
        trace.fraction, trace.startsolid
    );
    ...
    if spawn_points.len() >= 2 {
        ...
        println!(
            "  From spawn[0] ({:.0}, {:.0}, {:.0}) to spawn[3] ({:.0}, {:.0}, {:.0})",
